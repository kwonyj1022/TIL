    ...
    public void onClick (View v) {
        switch (v.getId()) {
            case R.id.load_pic_url_btn:
                ...
                // 4. 엑스트라에 1MB 이상의 데이터 전송
                byte[] byte1mb = new byte[1024 * 1024];
                intent.putExtra("DATA_1MB", byte1mb);

                // 5. 사진 불러오기 B 액티비티를 실행한다.
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == REQUEST_CODE_PIC_FILE_URL) {
            if (resultCode == RESULT_OK) {
                ...
                Toast.makeText(this, "액티비티 결과 취소", Toast.LENGTH_LONG).show();
            }
        }
    }
}